...
(see the repository root `AGENTS.md` for the full deployment architecture).
...
- **Post** (`/post/:slug`) — `GET /blogs/{slug}`, rendered from markdown to
  ...
  `.proto` the backend's own `entity.AllowedEmojis` reads (see `packages/protos/AGENTS.md`'s

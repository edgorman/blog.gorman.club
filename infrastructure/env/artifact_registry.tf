...
# `.github/CLAUDE.md`), so this repository grows at the same rate whether or not the environment it lives in
...
# the Artifact Stores section of `infrastructure/CLAUDE.md` for how the count was chosen and what it does not
...
# infrastructure/root, per the IAM section of `infrastructure/CLAUDE.md`: a grant lives beside the resource it

...
4. Frontend image promotion now mirrors the backend's exactly, since the backend URL is runtime configuration rather than baked into the bundle (see `services/frontend/src/lib/config.ts`): `gcrane` copies the commit-SHA-tagged image already built and validated on staging into the `blog-gorman-club-prod` Artifact Registry, retagged with the formal release version. Images are never rebuilt for the frontend either. That image's static files are then extracted and deployed to the production subdomain on Cloudflare Pages, with `config.json` written next to them from the just-promoted backend URL - the one step with no backend analogue, since a static site has no runtime environment of its own to read a URL from.

### Rollback Strategy

Promoting a previous release tag re-runs the same promotion flow: `terraform apply` using that release's infrastructure configuration, then shifting Cloud Run traffic back to its already-promoted backend image tag in `blog-gorman-club-prod` (the `gcrane` copy is harmless to repeat, since that version was promoted previously). The frontend's rollback is the same flow applied to its own promoted image: `gcrane` re-copies that release's tag (again a no-op) and its static files are re-extracted and redeployed to Cloudflare Pages with that release's own `config.json` - no rebuild, for either service, ever.

...
- **Alert policies** — Request health is watched on Cloud Run's own metrics rather than log-based metrics, so nothing has to be kept in step with the handlers: the uptime check failing from more than one region (a single failing checker is more likely to be that checker), more than N `5xx` responses in a five-minute window (counted rather than rated, since at this traffic a rate reads as noise), a spike of `4xx` responses, any real number of `429`s (the rate limiter turning someone away), and 95th-percentile latency above a threshold set well clear of a cold start.
- **Writing assistant** — The one route that spends money, and one Cloud Run cannot tell apart from any other request. The backend writes exactly one structured `assistant turn` log line per turn (outcome - ok, error, or canceled when the caller went away - the model API's status on failure, rounds, token counts, never content or a uid), and two log-based metrics count it: turns by outcome and upstream status, and tokens per turn. Alerts fire on turns in an hour above a threshold and on any failed turn, naming its upstream status. The metric filter matches the line's message exactly, so it and `assistantTurnMessage` in `services/backend/internal/service/chat.go` change together.
- **Dashboard** — One per environment (`backend-<env>`) with requests by class, p95 latency, assistant turns, failures and tokens, and instance count, so an alert has somewhere to click through to.
- **Thresholds** — Every one is a variable in `env/variables.tf` with a default and a description, and every policy carries a `documentation` block saying where to look first.

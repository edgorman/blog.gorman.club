...
**Codegen is committed, and CI fails on drift**, deliberately diverging from Pants' default of ephemeral, sandbox-only codegen: ephemeral output means `go mod tidy` errors on generated packages and gopls can't resolve them, needing `pants export-codegen ::` as a workaround for every editor session. Committing costs one CI check (`protos-drift` in `pull-request.yaml`: `buf generate` re-run against the tree, then `git diff --exit-code`) and keeps every editor and Go tool working normally with no extra step. The consequence is the reverse of the usual Pants convention in this repo: `services/backend/internal/gen` is committed, ordinary first-party Go source that Pants' Go dependency inference reads like any other package (`go_package()` in its own `BUILD`, same as every other package under `internal/`) — it is not modeled as codegen output at all.

**Generated code follows its consumer, not the contract it came from** — the same rule the Repository Structure section above states for a future `packages/go`: the `.proto` is the shared thing, while `services/backend/internal/gen` is build output committed for the sake of gopls and `go build`, so it lives inside the module that compiles it rather than in a module of its own. `option go_package` on every proto therefore points directly into `github.com/edgorman/blog.gorman.club/services/backend/internal/gen/...`, resolved by `buf`, not by Pants.

**Every tool version is pinned exactly** — `buf` itself (`bufbuild/buf-setup-action`, SHA-pinned to `v1.47.2` in `pull-request.yaml`, matching the binary used locally) and the `protoc-gen-go` plugin (`packages/protos/buf.gen.yaml` invokes `go run google.golang.org/protobuf/cmd/protoc-gen-go@v1.36.12`, the exact version `services/backend/go.mod` requires). An unpinned version would make the drift check fail on PRs that never touched a `.proto`, which is exactly the kind of noise that gets a check disabled rather than fixed.
...
This is intentionally the smallest possible slice: one message (`blog.v1.AllowedEmojis`, mirroring `entity.AllowedEmojis` in `services/backend/internal/entity/emoji.go`) with exactly one backend call site, proving the generate-and-verify pipeline end to end rather than migrating the entity model. Migrating further messages, generating TypeScript bindings, and giving Go a second module of its own (`packages/go`, once a second Go consumer of shared code exists — see Repository Structure above) are all deliberately left for follow-up work.

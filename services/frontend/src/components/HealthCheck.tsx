...
export function HealthCheck() {
  ...
  useEffect(() => {
    ...
    fetchHealth(BACKEND_URL, controller.signal)
      ...
      .catch((error: unknown) => {
        ...
        setState({ phase: 'error', message: errorMessage(error, 'Unknown error') })
      })
  ...
  return (
    <section className="panel" data-phase={state.phase}>
      <h2>Backend status</h2>

      {state.phase === 'unconfigured' && (
        <p>
          No backend deployed yet. Once <code>VITE_BACKEND_URL</code> is set, this card will call
          the backend's <code>/debug</code> endpoint on page load.
        </p>
      )}

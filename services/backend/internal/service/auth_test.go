...
func TestRequireAuth_MissingHeader(t *testing.T) {
	called := false
	handler := requireAuth(fakeVerifier{uid: "user-1"}, func(http.ResponseWriter, *http.Request) {
		called = true
	})
...
func TestRequireAuth_InvalidToken(t *testing.T) {
	handler := requireAuth(fakeVerifier{err: errors.New("bad token")}, func(http.ResponseWriter, *http.Request) {
...
func TestRequireAuth_Valid(t *testing.T) {
	var gotUID string
	handler := requireAuth(fakeVerifier{uid: "user-1"}, func(_ http.ResponseWriter, r *http.Request) {
...
func TestRequireAuth_UnconfiguredIsServerError(t *testing.T) {
	handler := requireAuth(fakeVerifier{err: repository.ErrAuthNotConfigured}, func(http.ResponseWriter, *http.Request) {

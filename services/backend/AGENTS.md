...
- **Events.** Eventarc triggers (`infrastructure/env/worker.tf`) deliver `google.cloud.firestore.document.v1.written` on `blogs/{slug}` to `POST /events/blog` and `...document.v1.created` on `blogs/{slug}/comments/{id}` to `POST /events/comment`, as binary-mode CloudEvents. Their body is a protobuf `DocumentEventData` (Firestore events offer no JSON encoding; setting `event_data_content_type = "application/json"` makes trigger creation fail), and the worker doesn't read it: `Ce-Type` and `Ce-Document` name the event and document, and a handler that needs the contents reads the document from Firestore.

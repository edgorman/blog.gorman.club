 ...
 *
 * Wire types come from `src/gen` rather than being declared here - see packages/protos/CLAUDE.md's "Contract
 ...
 * than derived from `Blog['visibility']`: the proto field is a plain string (see `packages/protos/CLAUDE.md`'s
...
async function request<T>(
  ...
  if (!response.ok) {
    // Every non-2xx response is an ErrorResponse (see `packages/protos/CLAUDE.md`'s "Contract Layer"), but fall back
...
export function createApi(baseUrl: string, authHeaders: AuthHeaders) {
  return {
    ...
    // only marshal a message at the top level (see `packages/protos/CLAUDE.md`'s "Contract Layer") - so the list is

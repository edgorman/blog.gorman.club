...
The backend is there now: Staging Deployments (below) pushes the SHA-tagged image to both registries on every merge, and Production Releases deploys straight from `blog-gorman-club-prod`'s own registry rather than copying into it. The frontend hasn't moved yet - Staging Deployments still builds and pushes only to staging's store, and Production Releases still fills prod's by copying staging's validated image across the project boundary with `gcrane`. That copy is what having a store in every project from the start makes unnecessary: once the frontend's build job also publishes to both, its promotion stops reading across the boundary at all and becomes nothing but a deploy - each project's store written once, by the job that built it, and never read from the other.
...
3. Frontend image promotion, since the backend URL is runtime configuration rather than baked into the bundle (see `services/frontend/src/lib/config.ts`): `gcrane` copies the commit-SHA-tagged image already built and validated on staging into the `blog-gorman-club-prod` Artifact Registry, retagged with the formal release version. Images are never rebuilt for the frontend. That image's static files are then extracted and deployed to the production subdomain on Cloudflare Pages, with `config.json` written next to them from the just-promoted backend URL - the one step with no backend analogue, since a static site has no runtime environment of its own to read a URL from.

### Rollback Strategy

Promoting a previous release tag re-runs the same promotion flow: `terraform apply` using that release's infrastructure configuration, then deploying Cloud Run in `blog-gorman-club-prod` straight to that release's own commit-SHA image tag - already sitting in prod's registry since the commit was built, so the deploy is harmless to repeat. The frontend's rollback is the same flow applied to its own promoted image: `gcrane` re-copies that release's tag (again a no-op) and its static files are re-extracted and redeployed to Cloudflare Pages with that release's own `config.json` - no rebuild, for either service, ever.

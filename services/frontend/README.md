...
- **Post** (`/post/:slug`) — `GET /blogs/{slug}`, rendered from markdown to
  ...
  typed off `gen/blog/v1/emoji.ts`, generated by `buf generate` from the same
  `.proto` the backend's own `entity.AllowedEmojis` reads (see `packages/protos/CLAUDE.md`'s
  "Contract Layer" section); widening the set means changing the proto and
  regenerating, not editing two hand-written arrays.
...
(`pants --changed-since=origin/main lint check test`, see .github/CLAUDE.md's "Building with Pants").

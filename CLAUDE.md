...
- **Notification channels** — One email channel per address in `alert_notification_emails`, kept separate rather than combined so a later policy can notify a subset without duplicating a channel. This list grants nothing; it is where a message goes, not an account admitted — nothing in the environment admits an account at all, since the assistant is entitled by each account's own subscription.

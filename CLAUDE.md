...
`ts-proto`'s `opt` flags each earn their keep rather than defaulting: `esModuleInterop=true` matches `tsconfig.app.json`'s `verbatimModuleSyntax`, which rejects ts-proto's default `import * as` star import; `useOptionals=messages` makes every field plain-optional (`field?: string`) instead of also nullable, matching proto3's own "unset reads as the zero value" semantics; `forceLong=string` decodes 64-bit integer fields to `string` rather than pulling in the `long` npm package for a `Long` class this repo has no other use for; `outputServices=false` is a no-op today (there is no `service` in any `.proto` here, only messages) but states plainly that a gRPC client is not what this layer is for — the backend is reached over plain REST. `strategy: all` on the ts-proto plugin entry is required by ts-proto under buf; it fails in confusing ways under buf's default per-directory invocation.
...
This is intentionally the smallest possible slice: one message (`blog.v1.AllowedEmojis`, mirroring `entity.AllowedEmojis` in `services/backend/internal/entity/emoji.go` and now `AllowedEmojis['emoji']` in `services/frontend/src/components/ReactionBar.tsx`) with exactly one call site per language, proving the generate-and-verify pipeline end to end rather than migrating the entity model. Migrating further messages, generating bindings for a second frontend consumer, and giving Go a second module of its own (`packages/go`, once a second Go consumer of shared code exists — see Repository Structure above) are all deliberately left for follow-up work.
